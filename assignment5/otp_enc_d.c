/*
    ...
    Date: 03/18/2017
    Description: Server for OTP encryption
*/
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
int encrypt(int);
char* oneTimeEncryption(char*);
char* generateCipher(char*, char*);
int verify(int);
        ...
        if (newConnectionFD > 0) {
            ...
            switch (spawnPid) {
                case 1:
                    ...
                    break;
                case 0:
                    // Child proces: encrypt the code
                    status = encrypt(newConnectionFD);
...
/*
    ...
    Return type: int
    Description: Encrypt the text with the key received,
                & then send the cipher text back to client
*/
int encrypt(int newConnectionFD) {
    ...
    // Get the cipherText from enpryption function
    char* cipherText = oneTimeEncryption(text);
    // Reset bufer
    memset(buffer, '\0', BUFFERLENGTH + 1);

    // Copy the cipherText to buffer in chunks, and send them until all texts are sent
    while (i < strlen(cipherText)) {
        ...
        if ((i + 1) % BUFFERLENGTH == 0 || cipherText[i + 1] == '\0') {
...
/*
    Function Name: oneTimeEncryption()
    Argument: char* mergedText
    return value: char*
    Description: split the mergedText into text and key, then encrypt
*/
char* oneTimeEncryption(char* mergedText) {
    ...
    // Generate the cipher text and return
    char* cipherText = generateCipher(text, key);
    return cipherText;
}

/*
    Function Name: generateCipher
    Argument: char* text, char* key
    return value: char* cipherText
    Description: Use the text and key to generate cipher text
*/
char* generateCipher(char* text, char* key) {
...
int verify(int newConnectionFD) {
    ...
    do {
        ...
        if (charsRead != 2) {
            ...
            if (strcmp(recvBuffer, "e") == 0) {
                // In this "if" statement if "e" is received, send 'y' to confirm
                do {
                ...
                // In this "else" statement if "d" is received, send 'n' to reject
                sendBuffer[0] = 'n';
                do {
                    charsWritten = send(newConnectionFD, sendBuffer, 2, 0);
                    if(charsWritten == 2) {
